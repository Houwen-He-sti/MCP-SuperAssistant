...
export interface BuildReadOnlyNotionBridgePromptOptions {
  ...
  assumeDefaultToolsWhenNoLiveTools?: boolean;
}
...
function selectReadOnlyToolNames(options: BuildReadOnlyNotionBridgePromptOptions): string[] {
  ...
  return options.assumeDefaultToolsWhenNoLiveTools ? [...DEFAULT_READ_ONLY_NOTION_BRIDGE_TOOL_NAMES] : [];
}
  ...
  const toolList =
    toolNames.length > 0
      ? toolNames
          ...
          .join('\n')
      : '- (none detected from live provider; wait for runtime tool sync before emitting jsonl)';
