...
describe('buildReadOnlyNotionBridgePrompt', () => {
  ...
  test('does not advertise tool names when runtime tools are unavailable', () => {
    const prompt = buildReadOnlyNotionBridgePrompt();
    ...
    for (const toolName of CURRENT_READ_ONLY_TOOLS) {
      assert.equal(
        ...
        false,
        `prompt should not advertise ${toolName} without live tools`,
      );
    }
    assertNoWriteToolNames(prompt);
  });

  test('can opt into fallback tool advertisement when explicitly configured', () => {
    const prompt = buildReadOnlyNotionBridgePrompt({ assumeDefaultToolsWhenNoLiveTools: true });
  ...
  test('filters out stale or write-capable tools from mixed input', () => {
    const prompt = buildReadOnlyNotionBridgePrompt({
      tools: [{ name: 'echo' }, { name: 'comment_on_pr' }, { name: 'merge_pr' }, { name: 'get_task_status' }],
    });
...
describe('NotionAdapter first-conversation prefix source', () => {
  test('uses the sanitized read-only prompt builder instead of the legacy static template', () => {
    ...
    assert.equal(source.includes('assumeDefaultToolsWhenNoLiveTools'), false);
  });
});
