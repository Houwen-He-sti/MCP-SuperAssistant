 ...
 *         asserts `enableStreamBridgeOnWindow(window)` is gated by
 *         `!this.bhBridgeDisposable` — lane-aware structure).
...
// ---------------------------------------------------------------------------
// T-BH-4-03 (P1-2): Source-contract test — lane-aware activate() call site lock
// Verifies:
//   1. enableStreamBridgeOnWindow(window) is called within isSupported() block
//   2. Call is gated by !this.bhBridgeDisposable (lane-aware: no dual execution)
//   3. startNotionRuntimeBridgeIfEnabled is called BEFORE the bridge enable
// Regression: removing lane gate or swapping order fails this test immediately.
// ---------------------------------------------------------------------------
describe('notion.adapter.ts source contract', () => {
  it('T-BH-4-03: activate() calls enableStreamBridgeOnWindow(window) lane-aware inside isSupported() block', () => {
    ...
    const activateBody = adapterSrc.slice(activateIdx, activateIdx + 3000);
    ...
    // BH-4 RuntimeBridge must be started before stream bridge enable
    const bhBridgeIdx = activateBody.indexOf('startNotionRuntimeBridgeIfEnabled(');
    assert.ok(bhBridgeIdx !== -1, 'startNotionRuntimeBridgeIfEnabled must be called in activate()');
    assert.ok(bhBridgeIdx > isSupportedIdx, 'startNotionRuntimeBridgeIfEnabled must be inside isSupported() block');

    // Lane gate must exist: enableStreamBridgeOnWindow must be guarded by !this.bhBridgeDisposable
    const laneGateIdx = activateBody.indexOf('!this.bhBridgeDisposable');
    assert.ok(laneGateIdx !== -1, '!this.bhBridgeDisposable gate must exist in activate() (lane-aware)');

    // enableStreamBridgeOnWindow(window) must appear AFTER both isSupported() and lane gate
    ...
    assert.ok(
      ...
      'enableStreamBridgeOnWindow(window) must appear inside the isSupported() guard',
    );
    assert.ok(
      bridgeCallIdx > laneGateIdx,
      'enableStreamBridgeOnWindow(window) must appear after the !this.bhBridgeDisposable lane gate',
    );
  });
});
