import {
  ...
  assembleNotionBridgePrompt,
  wrapWithSystemPromptTag,
} from '../../components/sidebar/Instructions/promptTemplateLoader';
...
export class NotionAdapter extends BaseAdapterPlugin {
  ...
  private readonly selectors = {
    ...
    // Confirmed selector from BH-1 DOM probe (2024): .layout-content is the
    // Notion AI conversation scroller. Old selectors (.notion-ai-chat-content,
    // [data-testid="ai-chat-content"]) returned 0 results and are stale.
    NATIVE_CHAT_CONTENT: '.layout-content, .layout-chat',
  ...
  private refreshBridgePromptCache(): void {
    try {
      ...
      this.cachedBridgePrompt = wrapWithSystemPromptTag(assembleInstructions({ platform: 'notion', tools: toolDefs }));
      this.context?.logger.debug(`[Slice1] Dynamic bridge prompt cached (${toolDefs.length} tools).`);
    } catch (err) {
      ...
      this.context?.logger.warn(`[Slice1] Failed to build dynamic bridge prompt, using static fallback: ${err}`);
      this.cachedBridgePrompt = null;
    }
  }
  ...
  async insertText(text: string, options?: { targetElement?: HTMLElement }): Promise<boolean> {
    ...
    if (!target) {
      ...
      const allSelectors = [...this.selectors.NATIVE_CHAT_INPUT.split(', '), ...this.selectors.CHAT_INPUT.split(', ')];
  ...
  getInputContent(): string | null {
    const selectors = [...this.selectors.NATIVE_CHAT_INPUT.split(', '), ...this.selectors.CHAT_INPUT.split(', ')];
    ...
    if (result.ok) {
      ...
      const reasonMsg =
        result.reason === 'button_disabled'
          ? 'Send button is disabled or detached'
          ...
          for (let j = 0; j < mutation.addedNodes.length; j++) {
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (el.textContent && el.textContent.trim().length > 10) {
                ...
                // DOM-based tool trigger: scan message text for ACK nonces and
                // ```jsonl function_call blocks (non-stream path, Gate 5d follow-up).
                // Deduplication handled inside mcpNotionDomScan.scan().
                const domScanner = (window as Record<string, unknown>).mcpNotionDomScan as
                  { scan?: (text: string) => void } | undefined;
                if (typeof domScanner?.scan === 'function') {
                  domScanner.scan(el.textContent);
                }
