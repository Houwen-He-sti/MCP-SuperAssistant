...
export class NotionAdapter extends BaseAdapterPlugin {
  ...
  isSupported(): boolean {
    ...
    return path === '/ai' || path.startsWith('/ai?') || path.startsWith('/ai/');
  }
  ...
  async insertText(text: string, options?: { targetElement?: HTMLElement }): Promise<boolean> {
    ...
    try {
      ...
      // Notion uses a contenteditable div (not ProseMirror).
      // Clear & re-insert so React-style state stays in sync.
      ...
      // Fire events so Notion's React layer picks up the change
    ...
    const stateManager = {
      getState: () => {
        try {
          ...
          return { mcpEnabled: false, autoSubmitEnabled: false, adapterName };
        }
      },
      toggleMCP: () => {
        try {
          const uiStore = context.stores.ui;
          if (uiStore && typeof uiStore.toggleMCP === 'function') {
            uiStore.toggleMCP();
          }
        } catch (error) {
          context.logger.error('Error toggling MCP:', error);
        }
      },
      toggleAutoSubmit: () => {
        try {
          const uiStore = context.stores.ui;
          if (uiStore && typeof uiStore.toggleAutoSubmit === 'function') {
            uiStore.toggleAutoSubmit();
          }
        } catch (error) {
          context.logger.error('Error toggling auto-submit:', error);
        }
      },
    };
