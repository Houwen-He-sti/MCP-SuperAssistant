...
describe('deriveStoredBridgeConfig', () => {
    test('enables execution only when MCP and autoExecute are both true', () => {
        assert.deepStrictEqual(
            deriveStoredBridgeConfig({ mcpEnabled: true, preferences: { autoExecute: true } }),
            { enabled: true, cutoffEnabled: true },
        );
        assert.deepStrictEqual(
            deriveStoredBridgeConfig({ mcpEnabled: true, preferences: { autoExecute: false } }),
            { enabled: false, cutoffEnabled: false },
        );
        assert.deepStrictEqual(
            deriveStoredBridgeConfig({ mcpEnabled: false, preferences: { autoExecute: true } }),
            { enabled: false, cutoffEnabled: false },
        );
    });
