...
/**
 * Generate a short, unique ACK nonce for a given callId.
 * Format: ack_<truncated_callId>_<counter_base36>
 * Guaranteed < 50 chars.
 */
export function generateNonce(callId: string): string {
  const truncatedId = callId.slice(0, 20);
  const suffix = (nonceCounter++).toString(36);
  return `ack_${truncatedId}_${suffix}`;
}
