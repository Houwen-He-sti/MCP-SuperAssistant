...
import type { BridgeHandoffAckEvent, InjectOutcome, StreamToolExecutionEvent } from './streamToolBridge.ts';
import {
    normalizeToUiEvent
...
// --- Gate 6E: ACK latency passthrough ---

describe('normalizeToUiEvent — Gate 6E: ACK latencyMs passthrough', () => {
    test('model_ack_confirmed preserves latencyMs', () => {
        const raw = makeAckEvent('model_ack_confirmed');
        const result = normalizeToUiEvent(raw);
        assert.ok(result);
        assert.equal(result.type, 'model_ack_confirmed');
    ...
    test('model_ack_timeout preserves latencyMs', () => {
        const raw = makeAckEvent('model_ack_timeout');
        const result = normalizeToUiEvent(raw);
        assert.ok(result);
        assert.equal(result.type, 'model_ack_timeout');
    ...
    test('non-ACK events do not have latencyMs', () => {
        const raw = makeStreamEvent('reserved');
        const result = normalizeToUiEvent(raw);
        assert.ok(result);
