...
async function findExtensionContext(cdp) {
  ...
  // There may be multiple MCP SuperAssistant contexts on different frames (iframes).
  ...
  // Find MCP SuperAssistant context on the same frame
  ...
  if (mainFrameId) {
    const onMainFrame = mcpCtxs.find(c => c.auxData?.frameId === mainFrameId);
    if (onMainFrame) return onMainFrame.id;
  }

  // Fallback: return any MCP context (last one is usually the main page)
  if (mcpCtxs.length > 0) return mcpCtxs[mcpCtxs.length - 1].id;
...
async function main() {
  ...
  // ===== Test 1: Verify extension interceptor installed =====
