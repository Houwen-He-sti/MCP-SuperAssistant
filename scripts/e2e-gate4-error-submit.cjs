/**
 * Gate 4 P0-3: Error Consumption Proof (Direct Injection Only)
 * Classification: CONSUMPTION PROOF — direct injection, not production bridge path.
 *
 * Injects an error-format function_result block directly via CDP adapter.insertText(),
 * submits via DOM click, and verifies AI echoes the error ID.
 * Does NOT test production bridge tool-failure → error-result auto-injection.
 * Production error auto-injection is deferred to Gate 5.
 */
