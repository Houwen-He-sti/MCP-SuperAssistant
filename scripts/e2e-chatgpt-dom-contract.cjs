...
(function() {
  ...
  // Check input and submit selectors
...
async function main() {
  ...
  try {
    ...
    const evidence = buildEvidence(scanA, scanB, stabilityReport);
...
function buildEvidence(scanA, scanB, stability) {
  ...
  return {
    ...
    submitButtonFound: scan.submitButtonFound,
...
function printSummary(evidence) {
  ...
  console.log(`  7. Submit button selector:        ${check(evidence.submitButtonFound)}`);
